...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered hcap address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HCAP.").arg(GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal hcap:URI (hcap:XyZ...?message=example)
            {
...
static void NotifyxHCAPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyxHCAPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyxHCAPReset.connect(boost::bind(NotifyxHCAPReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyxHCAPReset.disconnect(boost::bind(NotifyxHCAPReset, this));
}
