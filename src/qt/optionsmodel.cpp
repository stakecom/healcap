...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HCAP);
    ...
    if (!settings.contains("nAnonymizeHCAPAmount"))
        settings.setValue("nAnonymizeHCAPAmount", 1000);

    nAnonymizeHCAPAmount = settings.value("nAnonymizeHCAPAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHCAPAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeHCAPAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hcap.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHCAPAmount:
            return QVariant(nAnonymizeHCAPAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHCAPAmount:
            nAnonymizeHCAPAmount = value.toInt();
            settings.setValue("nAnonymizeHCAPAmount", nAnonymizeHCAPAmount);
            emit anonymizeHCAPAmountChanged(nAnonymizeHCAPAmount);
            break;
        case CoinControlFeatures:
