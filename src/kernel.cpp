...
bool GetKernelStakeModifierPos2(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval) {
        if (!pindexNext) {
            // Should never happen - FornaxA: **TODO** add Healcap's old stake modifier code
...
{
    //HCAP will hash in the transaction hash and the index number in order to make sure each hash is unique
